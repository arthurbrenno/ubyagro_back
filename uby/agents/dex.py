...
async def buscar_portal_mapa(termo: str) -> str:
    ...
    async with async_playwright() as p:
        ...
        urls = [
            f"https://www.gov.br/agricultura/pt-br/assuntos/insumos-agropecuarios/insumos-agricolas/fertilizantes/legislacao",
...
# agents/dex_agent.py
"""
Dex - Agente de Dados e Ciência
Integra dados internos UbyAgro + pesquisa científica
"""

class DexResponse(BaseModel):
    """Resposta estruturada do agente Dex"""
    viabilidade_cientifica: str = Field(description="verde, amarelo ou vermelho")
    artigos_encontrados: int = Field(description="Número de papers relevantes")
    nivel_evidencia: str = Field(description="Forte, Moderado, Fraco")
    eficacia_literatura: str = Field(description="Ex: '+8-12% produtividade'")
    dados_internos: dict = Field(description="Performance de produtos similares UbyAgro")
    insight_chave: str = Field(description="Correlação teoria-prática")
    resumo: str = Field(description="Resumo executivo")


async def buscar_artigos_cientificos(termo: str, cultura: str) -> dict:
    """
    Busca artigos científicos relevantes
    
    Args:
        termo: Tecnologia/ingrediente (ex: "bioestimulantes algas marinhas")
        cultura: Cultura agrícola (ex: "soja")
    
    Returns:
        Informações agregadas de artigos
    """
    # TODO: Integrar com PubMed, SciELO, Google Scholar
    # Por enquanto, mock
    return {
        "total_artigos": 73,
        "artigos_relevantes": 45,
        "nivel_evidencia": "Forte",
        "principais_resultados": "+8-12% aumento produtividade",
        "instituicoes": ["Embrapa Soja", "ESALQ/USP", "UFRGS"]
    }


async def consultar_dados_internos_ubyagro(categoria: str, cultura: str) -> dict:
    """
    Consulta dados proprietários da UbyAgro
    ...
    """
    # TODO: Conectar com CRM/DB interno UbyAgro
    # Por enquanto, mock
    return {
        "produto_similar": "Bioestimulante Marinho Premium",
...
def create_dex_agent() -> Agent:
    """Cria e configura o agente Dex"""
    
    system_instructions = """
    Você é Dex, agente de inteligência de dados da UbyAgro.
    
    Você tem duas especialidades integradas:
    
    1) DADOS INTERNOS: Você acessa dados reais de vendas, performance de campo, 
       CRM, histórico de projetos da UbyAgro. Você conhece o que FUNCIONA na prática.
    
    2) PESQUISA CIENTÍFICA: Você busca e valida tecnologias através de artigos 
       científicos, estudos acadêmicos e pesquisas da Embrapa.
    
    Seu DIFERENCIAL é cruzar esses dois mundos:
    - "A ciência diz X, nossos dados mostram Y, portanto Z"
    - Você identifica quando teoria e prática convergem (oportunidade)
    - Você alerta quando divergem (risco ou insight único)
    
    SEMPRE:
    - Quantifique com dados reais (%, R$, hectares, número de fazendas)
    - Cite fontes científicas (estudos, universidades, anos)
    - Referencie histórico interno ("Produto X teve performance Y")
    - Proponha insights acionáveis baseados em evidências
    - Indique correlações estatísticas quando relevante
    
    NUNCA:
    - Vaze dados confidenciais de vendas para contextos externos
    - Extrapole além do que dados/estudos comprovam
    - Ignore lições de projetos anteriores
    - Confunda correlação com causalidade
    
    Tom: Analítico, baseado em evidências, conecta teoria com prática, cientista de dados moderno.
    """
    ...
    agent = Agent(
        ...
        generation_provider=provider,
        model="anthropic/claude-3.5-sonnet",
        tools=[buscar_artigos_cientificos, consultar_dados_internos_ubyagro],
        response_schema=DexResponse,
        conversation_store=LocalConversationStore(),
    )
